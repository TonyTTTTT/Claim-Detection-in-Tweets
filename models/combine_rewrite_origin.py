...
rewrite_dataset = pd.read_csv('preprocess_datasets_GPT/CLEF2022_train_' + rewrite_method + '.tsv', sep='\t')

frames = [origin_dataset, rewrite_dataset]
concatenated_dataset = pd.concat(frames)

concatenated_dataset.to_csv('preprocess_datasets_GPT/CLEF2022_train_{}_augmented.tsv'.format(rewrite_method),
                            sep='\t', index=False)
