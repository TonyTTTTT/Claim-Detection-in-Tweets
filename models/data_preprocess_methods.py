...
def summary_by_GPT(ids, topic_ids, texts, labels, dataset):
    if os.path.exists('preprocess_datasets_tsv/{}_extract_by_GPT.tsv'.format(dataset)):
        data = pd.read_csv('preprocess_datasets_tsv/{}_extract_by_GPT.tsv'.format(dataset), sep='\t')
        ids, topic_ids, texts, labels = read_df_to_lists(data)
        print("load from preprocess_datasets_tsv/{}_extract_by_GPT.tsv".format(dataset))
        return ids, topic_ids, texts, labels
    ...
    df.to_csv('preprocess_datasets_tsv/{}_extract_by_GPT.tsv'.format(dataset), sep='\t', index=False)
