...
class EncoderAndDatabaseFactory(abc.ABC):
    """Abstract class to create the encoder and the database

    The output generated by the encoder should be able to query the database.
    For example, NNEncoderDatabase compute the similarity score between the query
    and the claims in the database. As all the claims was stored in tensor's form,
    the query should be a tensor too, so that their product can be computed.
